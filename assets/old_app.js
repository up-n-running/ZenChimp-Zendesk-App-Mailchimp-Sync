...
var appFactory = function( zafClient ) {
  ...
  return {
	...
	requests: 
	{
            ...
            {
                //secret oauth token
                //793f4120b84482abd6655565c6383775d1a72d2373dc5cb3463bef93ffe72716
                ...
                console.log( '%o', this );
                if( this.resources.DEBUG ) { console.log( "requests.getZendeskUser(%d), userApiCallSettings = %o", id, userApiCallSettings );}
	...
	init: function() 
	{
		...
		this.zendesk_user = null;
                ...
                //get location (which screen we're on)
                ...
                }, ( error ) => { switchToErrorMessage(error, "Could not get app context, please check your internet connection and try again");} );
		...
		//Get Settings from manifest.json
                this.v2Client.metadata().then( (metadata) => {
                    ...
                    this.resetAppIfPageFullyLoaded();
                }, ( error ) => { switchToErrorMessage(error, "Could not get your app settings, please check your internet connection and try again");} );
	},
//events: { 'app.activated': 'welcomeIfDataReady', 'ticket.subject.changed': 'welcomeIfDataReady', 'ticket.requester.email.changed': 'welcomeIfDataReady' },
	...
	resetAppIfPageFullyLoaded: function() 
	{
            ...
            { 
                ...
                console.log( "this.isFullyInitialized = " + this.isFullyInitialized + " and this.resources.APP_LOCATION_TICKET = " + this.resources.APP_LOCATION_TICKET + " and this.context.location = %s", this.context.location);
            }

            //has the context (ie which page are we on) loaded from init() function
            if( !this.isFullyInitialized && this.context === null )
            {
                if( this.resources.DEBUG ) { console.log( "CHECK FAILED: this.context === null" ); console.groupEnd(); }
                return;
            }   
            ...
            {
                if( this.resources.DEBUG ) { console.log( "CHECK FAILED: this.settings_fetched = false" ); console.groupEnd(); }
                return;
            } 
            ...
            {
                if( this.resources.DEBUG ) { console.log( "CHECK FAILED: this.zendesk_user === null, running this.v2Client.get('ticket.requester')" ); console.groupEnd();}
                this.v2Client.get('ticket.requester').then( (ticketRequester) => {
                    ...
                    console.log("NEED TO UPDATE this.zendesk_user = ticketRequester TO BE PROPER ZENDESK USER OBJECT");
                    this.resetAppIfPageFullyLoaded();
                }, ( error ) => { switchToErrorMessage(error, "Could not get the ticket info, please check your internet connection and try again");} );
                return;
            }
            ...
            {
                if( this.resources.DEBUG ) { console.log( "CHECK FAILED: this.user().id() = " + this.user().id() + " and this.user().email() = " + this.user().email() + " and this.user().name() = " + this.user().name() ); console.groupEnd();}
                return;
            }

            if( this.resources.DEBUG ) { console.log( "CHECK PASSED this.context = %o, this.settings_fetched = %o, this.context.location = %s, this.zendesk_user = %o", this.context, this.settings_fetched, this.context.location, this.zendesk_user ); console.groupEnd();}
            ...
            if(this.context.location === this.resources.APP_LOCATION_TICKET )
            {
                ...
                this.makeAjaxCall( this.requests.getZendeskUser( this.zendesk_user.id ), this.getZendeskUser_Done, this.switchToErrorMessage );
                //this jumps to getZendeskUser_Done assuming call was successful
            }
        ...
        makeAjaxCall: function (ajaxSettings, successFunction, failFunction) 
        {
            if( this.resources.DEBUG ) 
            { 
                console.group( "makeAjaxCall (ajaxSettings) called" );
            ...
            /*
            ajaxSettings.success = function (data, status, jqXhr) {
                if( this.resources.DEBUG ) { console.log( "AJAX SUCCESS: data = %o, status = %o, jqXhr = %o, function = %o", data, status, jqXhr, successFunction);}
                successFunction( data );
            };

            ajaxSettings.error = function (jqXhr, textStatus, errorMessage) {
                if( this.resources.DEBUG ) { console.log( "AJAX FAILURE: jqXhr = %o, textStatus = '%s', errorMessage = '%s', function = %o", jqXhr, textStatus, errorMessage, failFunction);}
                failFunction( textStatus, errorMessage );
            };
            
            var ajaxReq = $.ajax(ajaxSettings.url, {
                ajaxSettings
            });
            */
            ...
            this.v2Client.request(ajaxSettings).then(
                (data) => {
                    if( this.resources.DEBUG ) { console.log( "AJAX SUCCESS: data = %o", data ); }
                    successFunction.call( this, data );
                },
                (response) => {
                    if( this.resources.DEBUG ) { console.log( "AJAX FAIL: response = %o", response ); }
   ...
   //ZENDESK USER AND ORGANIZATION DATA API WRAPPER FUNCTIONS
	getZendeskUser_Done: function( userObjectFromDataAPI )
	{
            ...
            if( this.resources.DEBUG ) { console.log( "Converted userObjectFromDataAPI to user object: this.zendesk_user = %o", this.zendesk_user ); }
	...
	createZendeskUserFromAPIReturnData: function( userObjectFromDataAPI )
	{
            ...
            if( userObjectFromDataAPI !== null )
            {
                ...
                if( this.resources.DEBUG ) { console.log( "Completed part 1 of 2, basic user: zendeskUserObjectToReturn = %o", zendeskUserObjectToReturn); }
                ...
                if( this.resources.DEBUG ) { console.log( "Completed part 2 of 2, populateExtraFieldsFromUserAPIData: zendeskUserObjectToReturn = %o", zendeskUserObjectToReturn); }
	...
	fetchMailchimpObjectIfNecessary: function()
	{
            ...
            { 
                ...
                console.log( "this.zendesk_user.isIncluded() = %o, this.mailshot_sync_user = ", this.mailshot_sync_user );
            }
	...
	switchToMainTemplate: function() 
	{
            ...
            var formData = 
            {
                    ...
                    'monkey_URL': this.zendesk_user.isExcluded() ? "./exclude_monkey.png" : ( isInSync ? "./insync_monkey.png" :  "./outofsync_monkey.png" ),
                    'buttons': 
                    {
            ...
            if( this.resources.DEBUG ) { console.log( "Switching to template '%s' with form data: %o ", this.resources.TEMPLATE_NAME_MAIN, formData); }
            switchTo( this.resources.TEMPLATE_NAME_MAIN, formData );
